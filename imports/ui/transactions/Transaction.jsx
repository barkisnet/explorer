    ...
    render(){
        if (this.props.loading){
            ...
            if (this.props.transactionExist){
                ...
                return <Container id="transaction">
                    <Helmet>
                        <title>Transaction {tx.txhash} on Barkis Blockchain</title>
                    ...
                    {(tx.tx.value.msg && tx.tx.value.msg.length >0)?tx.tx.value.msg.map((msg,i) => {
                        return <Card body key={i}><Activities msg={msg} invalid={(!!tx.code)} events={tx.events} txhash={tx.txhash}/></Card>
                    }):''}
                </Container>
            }
            else{
