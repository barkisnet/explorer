...
export default class Activites extends Component {
    ...
    render(){
        ...
        case "cosmos-sdk/MsgSubmitProposal":
            return <p><Account address={msg.value.proposer} /> {(this.props.invalid)?<T>activities.failedTo</T>:''}<MsgType type={msg.type} /> <T>activities.withTitle</T> {(this.props.invalid)?msg.value.content.value.title:<Link to={"/proposals/"+this.props.events[2].attributes[0].value}>{msg.value.content.value.title}</Link>}<T>common.fullStop</T></p>
        ...
        case "cosmos-sdk/IssueMsg":
            return <p><MsgType type={msg.type} /> <Account address={msg.value.from} /> <T>messageTypes.IssueToken</T> <em className="text-warning">{msg.value.total_supply}</em> <em className="text-warning">{msg.value.symbol}({msg.value.name})</em> <T>common.comma</T> <em className="text-warning">decimal: {msg.value.decimal}</em> <T>common.comma</T> {(msg.value.mintable)?<T>activities.mintable</T>:<T>activities.unmintable</T>} <T>common.comma</T> <em className="text-warning">description: {msg.value.description}</em>  <T>common.fullStop</T> </p>
